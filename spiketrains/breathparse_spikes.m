function [spikerate_abs, b_rate, allbins_abs, allbins_raster winoverlap bin_abs raster_bin] = breathparse_spikes(breaths, events, spikes);
...
event_codes = [1 2 3]; %these are the event codes
...
seek_time = .1; %second to seek forward and backwards from event to find corresponding breath
...
num_bin_abs = ceil(.5/bin_abs); %number of bins to take on one side of the I/E
...
spikerate = zeros(bins*2,length(b_win),min_trials,length(event_codes));
spikerate_abs = zeros(num_bin_abs*winoverlap*2,length(b_win),min_trials,length(event_codes));
allbins_raster = int8(NaN(size(spikerate_abs,1)*((bin_abs/winoverlap)/raster_bin), length(b_win), min_trials, length(event_codes)));
    ...
    if SCALE_BINS == 1
        for t=1:min_trials; % number of trials
            ...
            for b = 1:length(b_win); %for each breath - this will bin the breath
                ...
                allbins(:,b,t,n) = (center_b-((bins+binoffset)*binsize)):(binsize/winoverlap):(center_b+((bins-binoffset)*binsize));
                for x = winoverlap:(size(allbins,1)-winoverlap-1)
                    s = find(spikes >= allbins(x,b,t,n) & spikes <= allbins(x+winoverlap,b,t,n));
                    numspikes = length(s);
                    clear s;
                    spikerate_abs(x,b,t,n) = numspikes/binsize; %aggregate as [bins, breaths, trials, events];
                end
            end
        end
    elseif SCALE_BINS == 0
        for t=1:length(sel_events_short); % number of trials
            ...
            for b = 1:length(b_win); %for each breath - this will bin the breath
                ...
                win_spikes = find(spikes_int >= min(abs_win)/raster_bin & spikes_int <= max(abs_win)/raster_bin); %find all spikes that fall within breath window
                if win_spikes %if any spikes in the window..
                    s = (spikes_int(win_spikes)-min(abs_win)/raster_bin); %how many raster bins before first spike
                    if s(1) == 0
                        s(1) = 1;
                    end
