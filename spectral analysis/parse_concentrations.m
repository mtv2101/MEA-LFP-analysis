...
maxpower_odor = squeeze(mean(maxpower(12:15,:,:),1)); %specify breaths with odor response
maxpower_norm = (maxpower_odor)-(maxpower_base);

sigmaxpower_base = squeeze(mean(sigmaxpower(1:10,:,:),1));
sigmaxpower_odor = squeeze(mean(sigmaxpower(12:15,:,:),1)); %specify breaths with odor response
