...
if sigtype == 0
    ...
    for i=1:size(all_gmean,2) %for each breath
        [sig_breaths(i),sig_vals(i),cis(i,:)] = ttest2(all_gmean(:,i),base_gmean(:),.01,'both','unequal'); %do one-tailed t-test
    end
    all_spec = all_gmean;
...
elseif sigtype == 1
    ...
    for i=1:size(all_gmax,2) %for each breath
        [sig_breaths(i),sig_vals(i),cis(i,:)] = ttest2(all_gmax(:,i),base_gmax(:),.01,'both','unequal'); %do one-tailed t-test
    end
    all_spec = all_gmax;
end