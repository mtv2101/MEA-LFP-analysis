...
%data = corr_breath; %[corr, corr, tirals, breaths]
tdt_map = [8,16,24,32;...
    7,15,23,31;...
    6,14,22,30;...
    5,13,21,29;...
...
for x=1:rows
    for y=1:cols
        indx = tdt_map(x,y);
        odormap = squeeze(mean(mean(corr_breath(indx,:,:,odor_win),4),3));
        basemap = squeeze(mean(mean(corr_breath(indx,:,:,base_win),4),3));
        subplot(rows,cols,(x*cols)-cols+y,'align');
        diffmap = (odormap-basemap)./basemap;
        %imagesc_mea(diffmap, 0, .3);
