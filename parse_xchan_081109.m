...
for n = 1:length(datafile); %n to number of channels
    ...
    for x = 1:length(brthindx) %x to num of breaths
        wave_segs(:,:,x,n) = parsechans_081109(wave,events,breaths,srate,eventcode,brthindx(x),winsize);
...
g_freqs = find(f>50 & f<100);
    ...
    [sig_breaths(:,n),sig_vals(:,n),cis(:,:,n),all_spec(:,:,n)] = test_breathsig_081109(spec_norm(:,:,:,:,n),brthindx,g_freqs,sigtype);
    %clear spec;
end